...
// .wrangler/tmp/bundle-KPo450/checked-fetch.js
...
// .wrangler/tmp/bundle-KPo450/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-KPo450/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var formatString = { format: (input) => input };
var formatCurrency = { format: (input) => toHumanReadable(input) };
var formatPercent = { format: (input) => `${parseFloat(input) <= 0 ? "\u{1F622}" : "\u{1F606}"} ${parseFloat(input).toLocaleString("en-US")}%` };
var formatPercentWoEmoji = { format: (input) => `${parseFloat(input).toLocaleString("en-US")}%` };
var formatBoolean = { format: (input) => !!input ? "Yes" : "No" };
...
var cmc_default = {
  async fetchSymbols(env, symbols) {
    try {
      ...
      tokens.forEach((token) => {
        data[token].filter((it) => it.is_active == true).forEach((tokenData) => {
          results.push(pullTokenData(tokenData));
        });
      });
      results.sort((a, b) => b.percentChange24h - a.percentChange24h);
      return results;
    } catch (error) {
      console.error(error);
      return [];
...
init_modules_watch_stub();
var slack_default = {
  async sendMessage(env, channel, text) {
    try {
...
// src/routes/slack/Prices.ts
var tokenArrayToMarkdownTable = (data) => {
  ...
  const columns = [readableColumnNames, ...data.map((token) => keys.map((column) => format(column, token[column])))];
  return markdownTable(columns);
};
var Prices_default = async (request, env) => {
  const body = await request.text();
  const params = import_qs.default.parse(body);
  const channel = params["channel_id"].trim();
  const symbols = params["text"].trim().replaceAll(" ", ",");
  const results = await cmc_default.fetchSymbols(env, symbols);
  const status = await slack_default.sendMessage(env, channel, "```\n" + tokenArrayToMarkdownTable(results) + "```\n");
  if (!status) {
...
// src/index.ts
var router = e();
router.post("/slack/prices", Prices_default);
router.get("/", hello_default);
router.get("*", __default);
var src_default = {
